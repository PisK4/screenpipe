...
- [T3 · Intent Agent 宿主形态](tickets/03-宿主形态.md): 选定 Rust 侧心跳 + 单次直调（B 方案），节拍与出卡判据分离，心跳只拉增量信号过门槛；上下文累积即查询窗口增长，不建新存储；随采集引擎常驻（不要求主窗口打开）；新手卡走 Tauri command 旁路；质量不够时生成段可升级为 Pi 会话，其余层不动。
