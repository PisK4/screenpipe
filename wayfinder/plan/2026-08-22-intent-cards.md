...
- 状态机五值 `proposed/shown/accepted/rejected/expired`，v1 只生效四条转移：`proposed→shown`、`shown→accepted`、`shown→rejected`、`shown→expired`；过期统一 shown 后 48h（T4/T8）。
...
票据留白、本计划锁定的实现口径，均有依据：

| # | 裁决 | 内容 | 依据 |
| --- | --- | --- | --- |
| D1 | 回退镜像机制 | chat active preset 存在前端 localStorage（`lib/active-ai-preset.ts`，键 `chat-active-preset-id`），心跳在主窗口关闭时读不到。前端在启动与切换 preset 时把解析后的 endpoint 三元组（url/model/api_key）经 command 镜像进 SettingsStore.extra 的 `intent_fallback_endpoint` 键。心跳取用顺序：intent 槽 → 镜像 → 内置 Ollama 默认。 | T7 两级回退 + T3 常驻语义的交集 |
| D2 | dedup_key 口径 | `{card_type}:{dominant_app}`；dominant_app 取 activity-summary 的首位 app 名，缺省 `general`。 | T4 决议 7「类型+核心对象」的最小实现 |
| D3 | 心跳节拍常量 | 间隔 900s，首轮延迟 120s；信号门槛：窗口内 app_switch+window_focus ≥ 3 或 frames hash 变化 ≥ 5。常量集中在 gate.rs 顶部，属可调参数。 | T3 决议 3「阈值是实现期调参项」的初值 |
| D4 | plans_json 结构 | `{"recommended_index": 0, "plans": [{"title","summary","consequence"}]}`；新手接入卡另用 `{"kind":"onboarding_mcp","detected_agents":[...]}`，前端按结构渲染，检测到的 agent 名为专有名词不进字典。 | T6 定稿结论 |
| D5 | 新手卡接受语义 | 「接入」= 写 accepted 终态 + 调 skills.rs 现成的 `connect_detected_ai_tools_in_background`，失败记 tracing 不回滚状态。不建新执行基础设施。 | T6 主按钮落在真实 MCP 注册机制 + T5「接受只写终态」收窄 |
...
pub async fn mark_shown(pool: &SqlitePool, id: i64, now: i64) -> Result<bool, sqlx::Error>; // 写 shown_at + expires_at = now+48h
...
逐函数实现到测试全绿。一个结构衔接点先确认：这些是自由函数（吃 `&SqlitePool`），而 src-tauri 侧经 `live_db()` 拿到的是 `Arc<DatabaseManager>`。落码时读 `db/setup.rs` 与 semantic 模块：若 DatabaseManager 有公开的池访问入口，自由函数照用；若没有，就把这组函数改成 `impl DatabaseManager` 的方法（semantic 模块同款风格），签名不变、只挪宿主。

两个时间格式注意点：
- `intent_cards` 自身时间列是 INTEGER unixepoch（chrono `Utc::now().timestamp()`）。
- `ui_events.timestamp` 与 `frames.timestamp` 是 SQLite DATETIME 文本列。传参格式照既有查询函数抄：`db/search.rs` 的 `get_frames_in_range_for_export` 怎么 bind 时间范围，这两个函数就怎么 bind。先读该函数确认格式再落码。

- [ ] **Step 4: 注册模块**

`db/mod.rs` 在 `mod memories;` 一组里插 `mod intent_cards;`，文件尾部加 `pub use self::intent_cards::{...};`（列出上面 Interfaces 全部公开项），风格与相邻 `pub use self::semantic::{...}` 一致。
...
Run: `bun scripts/pre_build.js && cargo test --manifest-path apps/screenpipe-app-tauri/src-tauri/Cargo.toml should_attempt`
Expected: 编译失败（函数未定义）。此后每个纯逻辑文件的测试先行步骤相同，不再重复写 Expected。

- [ ] **Step 2: 实现 gate.rs**

常量按 Interfaces 原文；`should_attempt_generation` 为两个 OR；`dedup_key` 空 app 归 `general`。
...
测试覆盖：围栏剥离、`insufficient_material:true` 分支、空 plans 报错、非法 card_type 报错、正常解析出 recommended_index。

- [ ] **Step 4: 写并实现 prompt.rs**

system 消息要点：角色说明；输出必须是单个 JSON 对象；三种 card_type 定义（light/side_effect/read_only）；plans 数组 1–3 项、首项为推荐项；材料不足时返回 `{"insufficient_material": true}`。user 消息放 `activity_summary` 的 JSON 文本（超长时先 `truncate_string(summary, 12_000)`，文件头加 `#![deny(clippy::string_slice)]`）与信号计数。测试断言 messages[0] 含 "JSON"、user 消息含传入的窗口起止文本。
...
git commit -m "feat(intent-agent): add generation gate, prompt builder, and model output parser"
```
...
pub fn resolve_endpoint(slot: Option<IntentModelEndpoint>, mirror: Option<IntentModelEndpoint>)
...
pub(crate) async fn read_supply(app: &tauri::AppHandle)
    -> (Option<model_call::IntentModelEndpoint>, Option<model_call::IntentModelEndpoint>); // (slot, mirror)，读 SettingsStore.extra
pub(crate) fn write_last_generation(app: &tauri::AppHandle, rec: &GenerationRecord); // 写 extra 的 intent_last_generation 键
```

- [ ] **Step 1: 核对两处现成签名再动笔**

读 `notifications/client.rs` 的 `send_typed_with_actions_and_priority` 参数表与 `NotificationPriority` 类型；读 `recording.rs::LocalApiContext` 与 `commands.rs::get_local_api_config` 的取用法；读 `notifications/gate.rs` 的 `load_guard` 看 SettingsStore.extra 的读取入口，并找它的写入侧函数。runner 的代码以下一步为准，签名不一致处以实测为准。
...
let window_start = latest_created_at 或 now-24h;               // 上下文累积即查询窗口增长（T3 决议 4）
let signals = WindowSignals {
    app_switches: count_ui_events_since(..).await?,
    frame_changes: count_frame_changes_since(..).await?,
};
if !gate::should_attempt_generation(&signals) { return Ok(None); } // 2. 门槛
let summary = fetch_activity_summary(app, &window_start).await?;  // 3. localhost GET :3030/activity-summary
let input = GenerationInput { ... };
let (slot, mirror) = read_supply(app);
let (ep, source) = model_call::resolve_endpoint(slot, mirror);
let record = match model_call::generate_card(&ep, &prompt::build_messages(&input)).await {
    Ok(raw) => match parse::parse_model_output(&raw) {
        ...
        Ok(ModelOutcome::Card(c)) => {
            ...
            let id = insert_intent_card(...).await?;                  // dedup 由 Task 1 兜底
            emit_intent_card_created(app, id);
            notify_new_card(app, id, &c).await?;                      // 4. notification-panel，闸门照单全收
...
`fetch_activity_summary`：GET `http://127.0.0.1:{port}/activity-summary?start_time={}&end_time={}&include_apps=true&include_windows=true`，Bearer api_key，30s timeout；query 时间格式先读 `crates/screenpipe-engine/src/routes/activity_summary.rs` 的 `ActivitySummaryQuery` 解析方式再定。
...
git commit -m "feat(intent-agent): add resident heartbeat loop with expiry settlement and panel notification"
```
...
pub struct IntentSupplyState { slot: Option<IntentModelEndpoint>, fallback_mirror: Option<IntentModelEndpoint>,
...
#[tauri::command] #[specta::specta] pub async fn intent_set_fallback_mirror(app: tauri::AppHandle, endpoint: Option<IntentModelEndpoint>) -> Result<(), String>;
```
...
- `intent_spawn_onboarding_card`（D5）：构造固定 plans（kind onboarding_mcp，detected_agents 从 `skills.rs` 的探测函数结果取；探测失败给空数组），origin=`system_onboarding`、dedup_key=`system_onboarding:onboarding`、expires_at=NULL，入库后 emit 事件并发面板提醒，返回 id。
- supply 三件套读写 SettingsStore.extra 的 `intent_slot` / `intent_fallback_mirror` / `intent_last_generation` JSON 键，读写入口以 Step 0 核对的 gate.rs 及其写入侧为准。
...
照侦察钉死的位置：`sidebar-nav-layout.ts` 的 SidebarNavId 联合类型加 `"workbench"`；`home/page.tsx` L61-74 ALL_SECTIONS、L803-815 SIDEBAR_SECTION_DEFS（label 用 `t("shell.nav.workbench")`，icon 选 lucide 的 `Sparkles`）、renderMainSection 加 `case "workbench": return <IntentWorkbench />;`。默认排位放 activity 之后。

- [ ] **Step 2: i18n 最小键集**

`en-intent-workbench.ts` 导出对象并入 en.ts 的 `workbench` 命名空间（zh 同构）。v1 键清单一次定齐，后续任务不再新增：
...
props: `plansJson: string`。解析 D4 结构；radio 列表，`recommended_index` 默认选中；选中项下方显示 consequence 行；末行虚线 radio + 输入框（原型 plan-compose 模式）：回车或失焦把输入钉成本地方案并选中、挂「你的方案」角标。轻提示卡不显示方案列表，只显示 proactive_view 正文。
...
- shown 回写：pending 列表渲染完成后对每张 `status==='proposed'` 的卡调 `intentCardMarkShown(id)`，组件内 `useRef<Set<number>>` 防重。
- finished 卡只读，显示终态徽标。
...
useTauriEvent<{ actionType?: string }>(NOTIFICATION_ACTION_EVENT, (e) => {
  if (e.actionType === "open_intent_workbench") setActiveSection("workbench");
});
```

事件名常量以 `events.rs` 的 `NOTIFICATION_ACTION_EVENT` 实值为准（侦察记录为既有转发通路）。面板「查看」action 的 kind 字段与 Task 4 发出的 action JSON 对齐。

- [ ] **Step 4: i18n 补卡片键**

`workbench.card.*`：accept/reject/knownOk/noThanks/customPlanBadge/yourPlan/recommended/consequenceLabel/status 五值/onboarding.title 等，en 先加、zh 补齐。按钮文案对照 T6 定稿：「知道了 / 不要」「接受选中方案 / 不要」。
...
**Interfaces:**
- Consumes: `commands.intentSetSlot / intentSetFallbackMirror / intentGetSupply / intentSpawnOnboardingCard`；`AIPresets`（use-settings.tsx）。

- [ ] **Step 1: 设置页 intent 槽卡片**

AISettings 内加一块卡片：preset 选择器枚举 `AIPresets`（选项含「跟随 Chat（默认）」，对应 slot=None）；选定后把该 preset 的 `{url, model, api_key}` 经 `intentSetSlot` 写入。下方一行展示 `intentGetSupply().last_generation`（最近成败 + 失败原因 + 来源 tag），失败不打扰工作台（T7 决议 3）。

- [ ] **Step 2: fallback 镜像（D1 关键步）**

`lib/active-ai-preset.ts`：`resolveActiveAiPreset` 结果变化与应用首次加载两个时机，把三元组经 `intentSetFallbackMirror` 上送；preset 带 apiKey 时照原样传（不出前端进程）。local-first 下镜像自然落到内置 Ollama preset。

- [ ] **Step 3: 新手卡接受触发点**

`intent_card_decide` 命中 accepted 且卡 origin 为 system_onboarding 时，tokio spawn 调 `skills.rs::connect_detected_ai_tools_in_background`，失败仅 tracing（D5）。
...
清单：设置页选 Ollama 槽 → 点「生成新手接入卡」→ 面板弹提醒（闸门静音时被抑制也算通过闸门验证）→ 点开进工作台 → 接受 → 状态变 accepted 且 skills 连接日志出现 → 把系统时间快进 49h 场景改为手动插一张 expires_at 已过的 shown 卡重启，验证心跳醒来先结算 expired。全程无账户登录。
...
| 上下文 = 查询窗口增长，不建存储（T3 决议 4） | Task 4 window_start 逻辑 |
| 主窗口新增 section，两个 Filter，新手卡随第一版（T5） | Task 6 |
| 到卡提醒复用 notification-panel 闸门（T5） | Task 4 notify_new_card |
| 五值状态机四条转移，写入唯一入口，过期挂心跳（T8） | Task 1/4/5 |
| 两级回退 + 内置 Ollama 默认，失败日志加设置页可见（T7） | Task 3/5/8 |
...
执行与终态链路、重写流程、记忆基底、记账三件套、BYOK 引导 UI、卡片快捷键、「执行中」Filter、Daily Summary 持久化。出现这些诉求时停下来说明，另行开票。
