...
export function NotificationPipeControls({
  ...
  if (loading && pipeRows.length === 0) {
    return (
      <p className="px-3 py-3 text-xs text-muted-foreground">{t("settings.notifications.pipesLoading")}</p>
    );
  }
  ...
  if (pipeRows.length === 0) {
    return (
      <div className="px-3 py-4 text-xs text-muted-foreground">
        {t("settings.notifications.pipesEmpty")}
      </div>
    );
  }
    ...
    <div className={cn("space-y-2", disabled && "opacity-50")}>
      <div className="flex items-center justify-between gap-2">
        <p className="text-[11px] text-muted-foreground">
          {mutedCount > 0
            ? t("settings.notifications.pipesMutedSummary", {
                muted: mutedCount,
                total: pipeRows.length,
              })
            : t("settings.notifications.pipesCanNotifySummary", { count: pipeRows.length })}
          {vipCount > 0 && (
            <span className="text-muted-foreground/80">
              {" · "}
              {t("settings.notifications.pipesAlwaysNotifyCount", { count: vipCount })}
            </span>
          )}
        </p>
        {mutedCount > 0 && (
          <button
            ...
            onClick={() => onChange([])}
          >
            {t("settings.notifications.unmuteAll")}
          </button>
        )}
      </div>
      {canVip && (
        <p className="text-[10px] text-muted-foreground/70">
          {t("settings.notifications.pipesVipHint")}
        </p>
      )}
      ...
      {pipeRows.length > SEARCH_THRESHOLD && (
        <div className="relative">
          ...
          <input
            ...
            placeholder={t("settings.notifications.pipesFilterPlaceholder")}
            aria-label={t("settings.notifications.pipesFilterPlaceholder")}
            disabled={disabled}
      ...
      <div className="divide-y divide-border border border-border">
        {filtered.length === 0 ? (
          <p className="px-3 py-3 text-center text-xs text-muted-foreground">
            {t("settings.notifications.pipesNoMatch", { query })}
          </p>
        ) : (
          filtered.map((row) => {
                ...
                <div className="flex shrink-0 items-center gap-2.5">
                  {canVip && (
                    <button
                      ...
                      aria-label={
                        isVip
                          ? t("settings.notifications.vipDisableAria", { name: row.title })
                          : t("settings.notifications.vipEnableAria", { name: row.title })
                      }
                      aria-pressed={isVip}
                      title={t("settings.notifications.vipTitle")}
