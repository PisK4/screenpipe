  ...
  return (
    <>
          ...
          >
            <div className="p-3 border-b border-border/50 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">{t("chat.history.title")}</span>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {groupedConversations.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {historySearch ? t("chat.history.noMatching") : t("chat.history.empty")}
                  </p>
                </div>
              ) : (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium truncate">
                            {(isInjectedTitle(conv.title) ? undefined : conv.title) || t("chat.history.untitled")}
                          </p>
                          <p className="text-[10px] text-muted-foreground">
                            {t("chat.history.messageCount", { count: conv.messageCount })}
                          </p>
                        </div>
                        <Popover
      ...
      <Dialog open={!!deletingConvId} onOpenChange={(open) => !open && setDeletingConvId(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <p className="text-sm text-muted-foreground">{t("chat.history.deleteConfirm")}</p>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={!!renamingConvId} onOpenChange={(open) => !open && setRenamingConvId(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>{t("chat.history.renameTitle")}</DialogTitle>
          </DialogHeader>
          <input
            autoFocus
